...
func doneMessage(t *testing.T, mode string) map[string]any {
	...
	// Downloaded as a volume, so one document covers the whole fixture and the
	...
	// which is true but a weaker test of the sentence. The source's theme is
	// the labelled one, because since the 2026-09-16 revision a volume is only
	// available where the source publishes volume labels.
	addVolumeSourceWithSplitStrips(t, store, mode)
